...
const levelBadgeClass = (level: AlertLevel) => {
    ...
    const c = ALERT_LEVEL_COLORS[level] ?? 'blue';
...
const AlertCard: React.FC<{
    ...
    return (
        <div
            className={`bg-white dark:bg-slate-900 rounded-2xl border-l-4 ${borderClass} p-5 flex gap-5 transition-all duration-150 hover:translate-x-0.5`}
            style={{
        ...
        >
            ...
            {canResolve && !alert.resolved && (
                <button
                    ...
                    className="shrink-0 flex items-center gap-2 min-h-[44px] px-5 py-2.5 rounded-xl text-sm font-semibold text-emerald-700 dark:text-emerald-400 bg-emerald-50 dark:bg-emerald-500/10 hover:bg-emerald-100 dark:hover:bg-emerald-500/20 transition-colors"
                >
        ...
        <div className="h-full flex flex-col bg-slate-50 dark:bg-slate-950 overflow-hidden">
            <div className="shrink-0 p-5 pb-4 border-b border-gray-100 dark:border-white/10 bg-white dark:bg-slate-900">
                <div className="flex items-center gap-3 mb-5">
                    ...
                    <h1 className="text-2xl font-black text-gray-900 dark:text-white">Alertas</h1>
                ...
                <div className="flex items-center gap-3 flex-wrap">
                    ...
                    <div className="relative flex-1 min-w-[180px]">
                        ...
                        <input
                            ...
                            className="w-full pl-10 pr-4 py-2.5 min-h-[44px] text-sm rounded-xl bg-gray-100 dark:bg-slate-800 border border-transparent focus:border-amber-400 focus:ring-0 text-gray-800 dark:text-white placeholder-gray-400 dark:placeholder-slate-500 focus:outline-none"
                        />
                    </div>
                    ...
                    <div className="flex items-center gap-1">
                        ...
                        {(['ALL', 'CRITICAL', 'WARNING', 'INFO'] as const).map(l => (
                            <button
                                ...
                                className="text-xs font-semibold min-h-[44px] px-4 py-2 rounded-xl transition-all duration-150 active:scale-95"
                                style={filterLevel === l ? {
            ...
            <div className="flex-1 overflow-y-auto p-5 space-y-4">
                {filtered.length === 0 ? (
                    <div className="py-24 text-center">
                        ...
                        <p className="text-gray-400 dark:text-slate-500 font-semibold text-base">No hay alertas activas</p>
                    </div>
                ) : (
                    filtered.map(alert => (
                        <AlertCard
