...
const StationCard: React.FC<StationCardProps> = ({
    ...
    return (
        <>
            ...
            {/* Card */}
            <div className="absolute bottom-4 left-4 right-4 lg:bottom-auto lg:top-20 lg:right-4 lg:left-auto lg:w-80 z-[500] pointer-events-auto animate-slide-up">
                <div
                ...
                >
                    ...
                    {/* Header */}
                    <div className="shrink-0 p-4 border-b border-gray-100/80 dark:border-white/8 bg-gray-50/50 dark:bg-white/[0.02]">
                        <div className="flex items-start justify-between gap-2">
                            <div>
                                <h2 className="font-black text-gray-900 dark:text-white text-base leading-tight">{station.name}</h2>
                                <div className="flex items-center gap-1 mt-0.5">
                                    <MapPin className="w-3 h-3 text-gray-400" />
                                    <p className="text-xs text-gray-400 dark:text-slate-500 truncate">{station.address}</p>
                                </div>
                            </div>
                            <button
                                onClick={onClose}
                                className="p-1.5 hover:bg-gray-100 dark:hover:bg-slate-800 rounded-full text-gray-400 shrink-0"
                            >
                                <X className="w-4 h-4" />
                            </button>
                        </div>
                        ...
                        {topAlert && (
                            <div className={`rounded-xl p-3 ${
                                ...
                                <div className="flex items-center gap-2 mb-1">
                                    ...
                                    <span className={`text-[10px] font-bold uppercase bg-${ALERT_LEVEL_COLORS[topAlert.level]}-100 dark:bg-${ALERT_LEVEL_COLORS[topAlert.level]}-500/20 text-${ALERT_LEVEL_COLORS[topAlert.level]}-700 dark:text-${ALERT_LEVEL_COLORS[topAlert.level]}-400 px-1.5 py-0.5 rounded-full`}>
                    ...
                    {/* Footer */}
                    <div className="shrink-0 p-3 border-t border-gray-100/80 dark:border-white/8 bg-gray-50/30 dark:bg-white/[0.02]">
                        <button
                            onClick={onViewDetails}
                            className="w-full flex items-center justify-center gap-2 py-2.5 rounded-2xl text-white font-bold text-sm transition-all active:scale-[0.98]"
                            style={{
